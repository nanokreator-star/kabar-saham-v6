...
# V6.7 — Rumor Intelligence scanner state.
...
# ============================================================
# V6.7 — RUMOR INTELLIGENCE SCANNER
# ============================================================
...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    print(
        "V6.7 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def scan():
    ...
    if getattr(
        ...
        try:
            rumor_articles = await core.fetch_all_rumor_articles()
...
async def test_telegram():
    ...
    if not rumor_regression_test.get("passed"):
        raise RuntimeError(
            "V6.7 Rumor Intelligence regression self-test gagal."
        )

    if not rumor_core_test.get("passed"):
        raise RuntimeError(
            "V6.7 Rumor Intelligence core self-test gagal."
        )

    if not rumor_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7 Rumor Intelligence scanner self-test gagal."
        )

    if not profile_sync_test.get("passed"):
        raise RuntimeError(
            "V6.7 profile sync self-test gagal."
        )
