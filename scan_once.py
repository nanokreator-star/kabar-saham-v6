"""
Kabar Saham V6.4 — Event Lifecycle & Corporate Action Timeline.
...
def _material_changes(previous, current):
    ...
    for key, label in (("money", "Nilai transaksi"), ("stake", "Stake")):
        ...
        if value and value != old:
            shown = ", ".join(str(x) for x in value[:3])
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.4 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def process_smart_watchlist(state, command_state, watchlist, articles, chat_ids):
    ...
    if SMART_LIFECYCLE_ALERT_ENABLED:
        for ticker, entry in watchlist.items():
            matches = core.find_ticker_matches(articles, ticker)
            if not matches:
                continue
            groups = core.group_event_articles(matches)
            for group in groups:
                article = group.get("primary")
                if not article:
                    continue
                try:
                    core.attach_official_reference(article, group.get("articles", []))
                ...
                previous = lifecycle_memory.get(key)
                merged = _merge_watch_snapshot(previous, snapshot)
                change_lines, kinds = _material_changes(previous, merged)
                priority = str(entry.get("priority") or "WATCH").upper()
                ...
                if _watch_should_alert(priority, kinds):
                    delivered = await send_to_all(
                        chat_ids,
                        _format_smart_change_alert(
                            ticker, entry, previous, merged, change_lines
                        ),
                    )
                    if delivered > 0:
                        lifecycle_memory[key] = merged
                    ...
                    # Persist only material baseline changes.
...
async def test_telegram():
    ...
    text = (
        "✅ <b>Kabar Saham V6.4 — SMART WATCHLIST + MILESTONE ALERT TEST OK</b>\n\n"
