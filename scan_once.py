...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    print(
        "V6.7.9 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _sanitize_rumor_memory_integrity(state):
    ...
    if repairs:
        ...
        print(f"V6.7.9 rumor integrity repaired: {repairs} field(s).")
...
def _reconcile_confirmed_rumor_memory_duplicates(state):
    ...
    if deduped:
        ...
        print(f"V6.7.9 confirmed rumor duplicates merged: {deduped} record(s).")
...
def _reconcile_rumor_memory_entity_keys(state, rumor_groups):
    ...
    if rekeyed:
        ...
        print(
            f"V6.7.9 rumor entity memory re-keyed: {rekeyed} record(s)."
        )
...
def _sync_rumor_memory_profiles_from_canonical_cache(state):
    ...
    for key, record in list(memory.items()):
        ...
        if synced != record:
            ...
            count += 1
    if count:
        print(f"V6.7.9 canonical ownership synced into rumor_memory: {count} record(s).")
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.7.9 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7.9 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def test_telegram():
    ...
    if not rumor_v679_test.get("passed"):
        raise RuntimeError(
            "V6.7.9 Unified Ownership Cache Sync core self-test gagal."
        )

    if not rumor_v679_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7.9 Rumor Memory Ownership Sync scanner self-test gagal."
        )
