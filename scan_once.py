...
# V6.4.1 — first-scan baseline guard + stable material normalization.
...
def load_state():
    ...
    # Transparent V6.4.1 state cleanup: normalize old duplicate stake/money.
    normalized_watch = {}
...
# ============================================================
# V6.4.1 SMART WATCH BASELINE + MATERIAL NORMALIZATION
# ============================================================
...
# ============================================================
# V6.4.1 SMART WATCHLIST SCANNER
# ============================================================
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.4.1 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.4.1 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def process_smart_watchlist(state, command_state, watchlist, articles, chat_ids):
    ...
    if SMART_LIFECYCLE_ALERT_ENABLED:
        for ticker, entry in watchlist.items():
            # V6.4.1: a newly added/updated watch gets ONE baseline-sync pass.
