...
# V6.7.11 — persistent Early Signal dedup/baseline.
EARLY_SIGNAL_RETENTION_DAYS = max(7, min(60, int(os.getenv("V6711_SIGNAL_RETENTION_DAYS", "30"))))
EARLY_SIGNAL_MAX_TICKERS_PER_SCAN = max(1, min(30, int(os.getenv("V6711_SIGNAL_MAX_TICKERS", "12"))))
EARLY_SIGNAL_RECENT_RUMOR_ALERT_MINUTES = max(5, min(120, int(os.getenv("V6711_RECENT_RUMOR_ALERT_MINUTES", "30"))))
...
def default_state():
    return {
        ...
        "early_signal_baseline_initialized": False,
        "early_signal_memory": {},
        "early_signal_alerts_sent": 0,
        "early_signal_duplicates_suppressed": 0,
        "early_signal_last_scan_utc": None,
    }
...
def load_state():
    ...
    data.setdefault("early_signal_baseline_initialized", False)
    if not isinstance(data.get("early_signal_memory"), dict):
        data["early_signal_memory"] = {}
    for counter_key in ("early_signal_alerts_sent", "early_signal_duplicates_suppressed"):
        try:
            data[counter_key] = int(data.get(counter_key, 0) or 0)
        except Exception:
            data[counter_key] = 0
    data.setdefault("early_signal_last_scan_utc", None)
...
def prune_state(state):
    ...
    early_memory = state.get("early_signal_memory") or {}
    if not isinstance(early_memory, dict):
        early_memory = {}
    early_cutoff = utc_now() - timedelta(days=EARLY_SIGNAL_RETENTION_DAYS)
    kept_early = {}
    for key, entry in early_memory.items():
        if not isinstance(entry, dict):
            continue
        stamp = parse_iso(entry.get("last_seen_utc")) or parse_iso(entry.get("last_alert_utc"))
        if stamp is None or stamp >= early_cutoff:
            kept_early[str(key)] = entry
    if kept_early != early_memory:
        state["early_signal_memory"] = kept_early
        changed = True
...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    for group in rumor_groups or []:
        ...
        sync_record = getattr(core, "sync_rumor_record_with_canonical_profile", None)
    ...
    print(
        "V6.7.11 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _sanitize_rumor_memory_integrity(state):
    ...
    if repairs:
        ...
        print(f"V6.7.11 rumor integrity repaired: {repairs} field(s).")
...
def _reconcile_confirmed_rumor_memory_duplicates(state):
    ...
    if deduped:
        ...
        print(f"V6.7.11 confirmed rumor duplicates merged: {deduped} record(s).")
...
def _reconcile_rumor_memory_entity_keys(state, rumor_groups):
    ...
    if rekeyed:
        ...
        print(
            f"V6.7.11 rumor entity memory re-keyed: {rekeyed} record(s)."
        )

    return changed, rekeyed


def _sync_rumor_memory_profiles_from_canonical_cache(state):
    """Silently repair persisted rumor_memory from the hydrated canonical profile cache."""
    memory = state.setdefault("rumor_memory", {})
    sync_one = getattr(core, "sync_rumor_record_with_canonical_profile", None)
    if not isinstance(memory, dict) or not callable(sync_one):
        return False, 0
    changed = False
    count = 0
    for key, record in list(memory.items()):
        if not isinstance(record, dict):
            continue
        synced = sync_one(record)
        if synced != record:
            memory[key] = synced
            changed = True
            count += 1
    if count:
        print(f"V6.7.11 canonical ownership synced into rumor_memory: {count} record(s).")
    return changed, count


def v679_rumor_memory_profile_sync_selftest():
    backup = copy.deepcopy(getattr(core, "ISSUER_PROFILE_CACHE", {}))
    try:
        core.ISSUER_PROFILE_CACHE.clear()
        core.ISSUER_PROFILE_CACHE["BYAN"] = {
            "ticker": "BYAN", "issuer_name": "Bayan Resources",
            "controller": {
                "name": "Dato' Dr. Low Tuck Kwong", "stake_pct": 40.22,
                "verified": True,
        ...
        state["rumor_memory"] = {key: {
            "ticker": "BYAN", "company_name": "Bayan Resources",
            "category": "AKUISISI / TAKEOVER", "status": "ACTIVE", "strength": "STRONG",
            "rumor_details": {}, "issuer_profile": {
                "ticker": "BYAN", "issuer_name": "Bayan Resources", "controller": None,
                "profile_source": "Yahoo Finance", "profile_source_quality": "FALLBACK",
            },
        }}
        changed, count = _sync_rumor_memory_profiles_from_canonical_cache(state)
        profile = state["rumor_memory"][key].get("issuer_profile") or {}
        controller = profile.get("controller") or {}
        return {
            "passed": all([
                changed, count == 1,
                "Low Tuck Kwong" in str(controller.get("name") or ""),
                float(controller.get("stake_pct") or 0) == 40.22,
            ]),
            "changed": changed, "count": count, "controller": controller.get("name"),
        }
    finally:
        core.ISSUER_PROFILE_CACHE.clear()
        core.ISSUER_PROFILE_CACHE.update(backup)
...
async def process_rumor_intelligence(
    ...
    profile_sync_changed, _ = _sync_rumor_memory_profiles_from_canonical_cache(state)
    ...
    state_changed = bool(
        rekey_changed or integrity_changed or confirmed_dedup_changed or profile_sync_changed
    )
    sent_count = 0
    current_keys = set()
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.7.11 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7.11 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
# ============================================================
# V6.7.11 — EARLY SIGNAL SCANNER / DEDUP
# ============================================================

def _freshest_active_rumor_for_ticker(state, ticker):
    ticker = str(ticker or "").upper()
    candidates = []
    for record in (state.get("rumor_memory") or {}).values():
        if not isinstance(record, dict):
            continue
        ...
        if str(record.get("status") or "ACTIVE").upper() != "ACTIVE":
            continue
        if not core.rumor_is_push_fresh(record):
            continue
        candidates.append(record)
    candidates.sort(key=lambda x: parse_iso(x.get("last_seen_utc")) or datetime.min.replace(tzinfo=timezone.utc), reverse=True)
    return candidates[0] if candidates else None


def _signal_signature(ticker, signal, rumor_record=None):
    level = str((signal or {}).get("level") or "NONE").upper()
    direction = str((signal or {}).get("direction") or "FLAT").upper()
    market_date = str((signal or {}).get("market_date") or "UNKNOWN")
    combined = "RUMOR" if rumor_record else "MARKET"
    rumor_key = str((rumor_record or {}).get("key") or "") if rumor_record else ""
    return f"{str(ticker).upper()}|{market_date}|{level}|{direction}|{combined}|{rumor_key}"


def _recent_rumor_alert(record):
    if not isinstance(record, dict):
        return False
    dt = parse_iso(record.get("last_alert_utc"))
    if dt is None:
        return False
    return utc_now() - dt <= timedelta(minutes=EARLY_SIGNAL_RECENT_RUMOR_ALERT_MINUTES)


def _early_signal_should_alert(previous, signal, *, combined=False):
    if not (signal or {}).get("active"):
        return False
    if not isinstance(previous, dict):
        return True
    old_level = str(previous.get("level") or "NONE").upper()
    new_level = str(signal.get("level") or "NONE").upper()
    rank = {"NONE": 0, "MEDIUM": 1, "HIGH": 2}
    if rank.get(new_level, 0) > rank.get(old_level, 0):
        return True
    old_date = str(previous.get("market_date") or "")
    new_date = str(signal.get("market_date") or "")
    if old_date and new_date and new_date != old_date:
        return True
    old_direction = str(previous.get("direction") or "FLAT").upper()
    new_direction = str(signal.get("direction") or "FLAT").upper()
    if old_direction not in {"", "FLAT"} and new_direction not in {"", "FLAT"} and old_direction != new_direction:
        return True
    if combined and not bool(previous.get("combined")):
        return True
    return False


async def process_early_signal_engine(state, watchlist, chat_ids):
    if not getattr(core, "EARLY_SIGNAL_ENGINE_ENABLED", False):
        return False, 0
    ...
    for record in (state.get("rumor_memory") or {}).values():
        ...
        if re.fullmatch(r"[A-Z]{4}", ticker) and str(record.get("status") or "").upper() == "ACTIVE" and core.rumor_is_push_fresh(record) and ticker not in tickers:
            tickers.append(ticker)
    tickers = tickers[:EARLY_SIGNAL_MAX_TICKERS_PER_SCAN]

    memory = state.setdefault("early_signal_memory", {})
    first_baseline = not bool(state.get("early_signal_baseline_initialized"))
    changed = False
    sent_count = 0

    for ticker in tickers:
        try:
            market = await core.get_market_data(ticker)
        except Exception:
            market = None
        if not market:
            continue
        rumor_record = _freshest_active_rumor_for_ticker(state, ticker)
        combined = rumor_record is not None
        signal = core.market_anomaly_signal(market, combined_context=combined)
        if not signal.get("active"):
            continue

        key = str(ticker).upper()
        previous = memory.get(key) if isinstance(memory.get(key), dict) else None
        signature = _signal_signature(ticker, signal, rumor_record if combined else None)
        ...
        entry = {
            ...
            "last_alert_utc": (previous or {}).get("last_alert_utc"),
        }

        # Upgrade baseline prevents historical market moves from flooding chat.
        if first_baseline:
            memory[key] = entry
            changed = True
            continue
        ...
        if previous and previous.get("signature") == signature:
            state["early_signal_duplicates_suppressed"] = int(state.get("early_signal_duplicates_suppressed", 0) or 0) + 1
            memory[key] = {**previous, **entry, "last_alert_utc": previous.get("last_alert_utc")}
            changed = True
            continue

        if combined and _recent_rumor_alert(rumor_record):
            # The just-delivered rumor alert already contains the EARLY SIGNAL
            # block, so a second Telegram message would be duplicate noise.
            memory[key] = entry
            state["early_signal_duplicates_suppressed"] = int(state.get("early_signal_duplicates_suppressed", 0) or 0) + 1
            changed = True
            continue

        if not _early_signal_should_alert(previous, signal, combined=combined):
            memory[key] = entry
            state["early_signal_duplicates_suppressed"] = int(state.get("early_signal_duplicates_suppressed", 0) or 0) + 1
            changed = True
            continue

        priority = (watchlist.get(ticker) or {}).get("priority") if ticker in watchlist else None
        text = core.format_market_signal_alert(ticker, market, rumor_record=rumor_record if combined else None, watch_priority=priority)
        if not text:
            continue
        delivered = await send_to_all(chat_ids, text)
        if delivered > 0:
            entry["last_alert_utc"] = utc_iso()
            memory[key] = entry
            state["early_signal_alerts_sent"] = int(state.get("early_signal_alerts_sent", 0) or 0) + 1
            sent_count += 1
            changed = True
            print("Early Signal alert delivered:", ticker, signal.get("level"), "combined=" + str(combined))

    if first_baseline:
        state["early_signal_baseline_initialized"] = True
        changed = True
        print("Early Signal baseline initialized; existing anomalies suppressed.")

    state["early_signal_last_scan_utc"] = utc_iso()
    return changed, sent_count


def v6711_early_signal_scanner_selftest():
    medium = {"active": True, "level": "MEDIUM", "direction": "UP", "market_date": "2026-08-20"}
    high = {"active": True, "level": "HIGH", "direction": "UP", "market_date": "2026-08-20"}
    previous = {"level": "MEDIUM", "direction": "UP", "combined": False, "market_date": "2026-08-20"}
    duplicate = _signal_signature("TEST", medium, None) == _signal_signature("TEST", medium, None)
    next_day = dict(medium); next_day["market_date"] = "2026-08-21"
    reversal = dict(medium); reversal["direction"] = "DOWN"
    checks = {
        "new_signal_alerts": _early_signal_should_alert(None, medium),
        "same_level_suppressed": not _early_signal_should_alert(previous, medium),
        "level_escalation_alerts": _early_signal_should_alert(previous, high),
        "new_market_day_alerts": _early_signal_should_alert(previous, next_day),
        "direction_reversal_alerts": _early_signal_should_alert(previous, reversal),
        "combined_upgrade_alerts": _early_signal_should_alert(previous, medium, combined=True),
        "signature_stable": duplicate,
    }
...
async def scan():
    ...
    early_changed, early_sent = await process_early_signal_engine(
        state,
        watchlist,
        chat_ids,
    )
    if early_changed:
        state_changed = True
    if early_sent:
        print(f"Early Signal alerts delivered: {early_sent}.")
...
async def test_telegram():
    ...
    rumor_v677_test = core.v677_ownership_safety_selftest()
    rumor_v678_test = core.v678_structured_ownership_selftest()
    rumor_v679_test = core.v679_unified_ownership_cache_sync_selftest()
    rumor_v6710_test = core.v6710_major_holder_display_sync_selftest()
    early_v6711_test = core.v6711_early_signal_selftest()
    early_v6711_scanner_test = v6711_early_signal_scanner_selftest()
    rumor_v679_scanner_test = v679_rumor_memory_profile_sync_selftest()
    rumor_v676_test = core.v676_ownership_finalization_selftest()

    if not early_v6711_test.get("passed"):
        raise RuntimeError(
            "V6.7.11 Early Signal Engine V9 core self-test gagal."
        )

    if not early_v6711_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7.11 Early Signal scanner dedup self-test gagal."
        )

    if not rumor_v6710_test.get("passed"):
        raise RuntimeError(
            "V6.7.10 Major Holder Display Sync V8 core self-test gagal."
        )

    if not rumor_v679_test.get("passed"):
        raise RuntimeError(
            "V6.7.10 Unified Ownership Cache Sync core self-test gagal."
        )

    if not rumor_v679_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7.10 Rumor Memory Ownership Sync scanner self-test gagal."
        )

    if not rumor_v677_test.get("passed"):
        raise RuntimeError(
            "V6.7.8 Ownership Safety Hotfix self-test gagal."
        )

    if not rumor_v678_test.get("passed"):
        raise RuntimeError(
            "V6.7.8 Structured Ownership Resolver self-test gagal."
        )

    if not rumor_v676_test.get("passed"):
        raise RuntimeError(
            "V6.7.8 Ownership Intelligence Finalization self-test gagal."
        )

    if not rumor_v675_test.get("passed"):
        raise RuntimeError(
            "V6.7.8 Issuer/Owner/Profile hotfix self-test gagal."
        )
    ...
    text = (
        ...
        "Ownership Safety V5 / Poisoned Cache Repair: ✅ PASS\n"
        "Structured Ownership Resolver V6: ✅ PASS\n"
        "Unified Ownership Cache Sync V7: ✅ PASS\n"
        "Major Holder Display Sync V8: ✅ PASS\n"
        "Early Rumor Detection V9: ✅ PASS\n"
        "Unusual Price/Volume Signal V9: ✅ PASS\n"
        "Combined Rumor + Market Signal V9: ✅ PASS\n"
        "Early Signal Dedup/Baseline Guard: ✅ PASS\n"
        "Holder Completeness Retry: ✅ PASS\n"
        "Rumor Memory Canonical Profile Sync: ✅ PASS\n"
        "Profile-only Ownership Sync No-Realert: ✅ PASS\n"
        "Exact GMS/Menu False-Positive Guard: ✅ PASS\n"
        "Strict Official/Primary Owner Parser: ✅ PASS\n"
        "Owner Intelligence V4 Regression: ✅ PASS\n"
        "Primary Company Deep Owner Resolver: ✅ PASS\n"
        "Reverse-Role Controller Parser: ✅ PASS\n"
        "Negative Affiliation Reference Guard: ✅ PASS\n"
        "Official Registry >5% Holder Parser: ✅ PASS\n"
        "V4 Fresh Cache Retry: ✅ PASS\n"
