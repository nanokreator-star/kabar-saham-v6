...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    print(
        "V6.7.7 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _sanitize_rumor_memory_integrity(state):
    ...
    if repairs:
        ...
        print(f"V6.7.7 rumor integrity repaired: {repairs} field(s).")
...
def _reconcile_confirmed_rumor_memory_duplicates(state):
    ...
    if deduped:
        ...
        print(f"V6.7.7 confirmed rumor duplicates merged: {deduped} record(s).")
...
def _reconcile_rumor_memory_entity_keys(state, rumor_groups):
    ...
    if rekeyed:
        ...
        print(
            f"V6.7.7 rumor entity memory re-keyed: {rekeyed} record(s)."
        )
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.7.7 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7.7 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def test_telegram():
    ...
    if not rumor_v677_test.get("passed"):
        raise RuntimeError(
            "V6.7.7 Ownership Safety Hotfix self-test gagal."
        )

    if not rumor_v676_test.get("passed"):
        raise RuntimeError(
            "V6.7.7 Ownership Intelligence Finalization self-test gagal."
        )

    if not rumor_v675_test.get("passed"):
        raise RuntimeError(
            "V6.7.7 Issuer/Owner/Profile hotfix self-test gagal."
        )
