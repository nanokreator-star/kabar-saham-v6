...
# V6.7.1 — Rumor Intelligence scanner state.
...
# ============================================================
# V6.7.1 — RUMOR INTELLIGENCE SCANNER
# ============================================================
...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    print(
        "V6.7.1 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _reconcile_rumor_memory_entity_keys(state, rumor_groups):
    ...
    for old_key, old_record in list(memory.items()):
        ...
        migrated["entity_resolution_source"] = "V6.7.1_MEMORY_REKEY"
    ...
    if rekeyed:
        ...
        print(
            f"V6.7.1 rumor entity memory re-keyed: {rekeyed} record(s)."
        )
...
async def process_rumor_intelligence(
    ...
    state_changed = bool(rekey_changed)
    sent_count = 0
    current_keys = set()
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.7.1 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7.1 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def test_telegram():
    ...
    if not rumor_entity_prime_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor Entity Multi-Source Prime self-test gagal."
        )

    if not rumor_entity_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor Entity Resolution self-test gagal."
        )

    if not rumor_rekey_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor UNKNOWN Memory Merge self-test gagal."
        )

    if not rumor_regression_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor Intelligence regression self-test gagal."
        )

    if not rumor_core_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor Intelligence core self-test gagal."
        )

    if not rumor_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 Rumor Intelligence scanner self-test gagal."
        )

    if not profile_sync_test.get("passed"):
        raise RuntimeError(
            "V6.7.1 profile sync self-test gagal."
        )
