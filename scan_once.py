...
# V6.7.3 — Rumor Intelligence scanner state.
...
# ============================================================
# V6.7.3 — RUMOR INTELLIGENCE SCANNER
# ============================================================
...
def _initialize_rumor_baseline(state, rumor_groups):
    ...
    print(
        "V6.7.3 rumor baseline initialized:",
        len(memory),
        "rumor record(s).",
    )
...
def _sanitize_rumor_memory_integrity(state):
    ...
    if repairs:
        ...
        print(f"V6.7.3 rumor integrity repaired: {repairs} field(s).")
...
def _reconcile_confirmed_rumor_memory_duplicates(state):
    ...
    if deduped:
        ...
        print(f"V6.7.3 confirmed rumor duplicates merged: {deduped} record(s).")
...
def _reconcile_rumor_memory_entity_keys(state, rumor_groups):
    ...
    for old_key, old_record in list(memory.items()):
        ...
        migrated["entity_resolution_source"] = "V6.7.3_MEMORY_REKEY"
    ...
    if rekeyed:
        ...
        print(
            f"V6.7.3 rumor entity memory re-keyed: {rekeyed} record(s)."
        )
...
def _format_smart_change_alert(ticker, entry, previous, current, change_lines):
    ...
    lines = [
        f"🚨 <b>V6.7.3 SMART WATCH — {ticker}</b>",
...
def _format_milestone_alert(ticker, entry, snapshot, key, parsed, days_until):
    ...
    lines = [
        f"⏰ <b>V6.7.3 MILESTONE ALERT — {ticker}</b>",
        f"{icon} Priority: <b>{priority}</b>",
        f"🏢 Issuer: {html.escape(issuer)}",
...
async def test_telegram():
    ...
    if not rumor_v673_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Source Quality / Anti-Copy / Trading Impact self-test gagal."
        )

    if not rumor_integrity_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Data Integrity core self-test gagal."
        )

    if not rumor_integrity_state_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Data Integrity state self-test gagal."
        )

    if not rumor_entity_prime_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Entity Multi-Source Prime self-test gagal."
        )

    if not rumor_entity_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Entity Resolution self-test gagal."
        )

    if not rumor_rekey_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor UNKNOWN Memory Merge self-test gagal."
        )

    if not rumor_regression_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Intelligence regression self-test gagal."
        )

    if not rumor_core_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Intelligence core self-test gagal."
        )

    if not rumor_scanner_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 Rumor Intelligence scanner self-test gagal."
        )

    if not profile_sync_test.get("passed"):
        raise RuntimeError(
            "V6.7.3 profile sync self-test gagal."
        )
    ...
    text = (
        ...
        "Anti-Copy / Mirror Cluster Guard: ✅ PASS\n"
        "Effective Independence Engine: ✅ PASS\n"
        "Indonesian Business Profile: ✅ PASS\n"
        "Safe Owner Fallback: ✅ PASS\n"
